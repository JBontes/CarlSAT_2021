...
#define DEBUGLINE() printf("%s:%d\n", __FILE__, __LINE__);
    ...
    */
    ...
    /* ################################################################### */
    static void ToS8(std::string** strings, int64_t* out, int n) {
        for (int i = 0; i < n; ++i) {
    ...
    /* ################################################################### */
    static void ToU8(std::string** strings, uint64_t* out, int n) {
        for (int i = 0; i < n; ++i) {
            std::stringstream ss(strings[i]->c_str());
            ss >> out[i];
        }
    ...
    */
    ...
    public:
        inline explicit ezOptionValidator(const char* _type, const char* _op = 0, const char* list = 0, bool _insensitive = false);
        inline explicit ezOptionValidator(char _type);
        ...
        inline ezOptionValidator(char _type, char _op, const int64_t* list, int _size);
        inline ezOptionValidator(char _type, char _op, const uint64_t* list, int _size = 0);
        ...
        inline bool isValid(const std::string* value) const;
        inline void print();
        inline void reset();
...
#define CLEAR(TYPE,P) case TYPE: if (P) delete [] P; P = 0; break;
    ...
    /* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const int64_t* list, int _size) : s8(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = ezOptionParserIDGenerator::instance().next();
        s8 = new int64_t[size];
        memcpy(s8, list, size * sizeof(int64_t));
    };
    /* ------------------------------------------------------------------- */
    ezOptionValidator::ezOptionValidator(char _type, char _op, const uint64_t* list, int _size) : u8(0), op(_op), quiet(0), type(_type), size(_size), insensitive(0) {
        id = ezOptionParserIDGenerator::instance().next();
...
#define ToArray(T,P,Y) case T: P = new Y[size]; To##T(strings, P, size); FreeStrings(); break;
    ...
    /* ------------------------------------------------------------------- */
    bool ezOptionValidator::isValid(const std::string* valueAsString) const {
...
#define CHECKRANGE(E,T) {\
  ...
  \
  int64_t E##max = static_cast<int64_t>(std::numeric_limits<T>::max()); \
  if (E##value > E##max) { \
    if (!quiet) \
...
#define VALIDATE(T, U, LIST) { \
  ...
  switch (op) {\
    ...
    case GTLE:\
      ...
      case NOOP: case IN: default: break;\
        ...
        switch (type) {
            ...
            case U8: VALIDATE(uint64_t, uint64_t, u8); break;
            case S8: VALIDATE(int64_t, int64_t, s8); break;
    ...
    public:
        ...
        inline void getBool(bool&);
        inline void getInt(int&);
        inline void getLong(long&);
        inline void getInt64(int64_t&);
        inline void getULong(unsigned long&);
        inline void getUInt64(uint64_t&);
    ...
    /* ################################################################### */
    void OptionGroup::getBool(bool& out) {
        if (!isSet) {
            if (defaults.empty())
                out = 0;
            else
    ...
    /* ################################################################### */
    void OptionGroup::getUInt64(uint64_t& out) {
        if (!isSet) {
            if (defaults.empty())
                out = 0;
            else {
    ...
    public:
        ...
        int isSet(const std::string& name);
    ...
    /* ################################################################### */
    int ezOptionParser::isSet(const std::string& name) {
    ...
    void ezOptionParser::getUsageDescriptions(std::string& usage, int width, Layout layout) {
        ...
        for (i = 0; i < (long int)groups.size(); ++i) {
            ...
            const auto helpwidth = width - pad;
            ...
            for (insertionIter = desc.begin(), cIter = insertionIter++;
                ...
                if ((long int)((*cIter)->size()) > helpwidth) {
                    ...
                    if (rem->size()) { desc.insert(insertionIter, rem); } else { delete rem; }
                }
            }
    ...
    bool ezOptionParser::gotExpected(std::vector<std::string>& badOptions) {
        for (int i = 0; i < (long int)groups.size(); ++i) {
            ...
            if (g->isSet) {
                ...
                for (int j = 0; j < (long int)g->args.size(); ++j) {
